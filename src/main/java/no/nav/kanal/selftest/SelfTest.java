...
public class SelfTest {
	...
	public SelfTest(String truststorePath, VaultCredentials credentials) {
	...
	private SelfTestResult testTrustStore() {
		...
		Status status = Status.OK;
		StringBuilder message = new StringBuilder("Path: " + truststorePath + "<BR>");
		String stacktrace = null;
		try {
			final KeyStore keyStore = KeyStore.getInstance("JKS");
			try(InputStream keystoreFileInputStream = new FileInputStream(truststorePath)) {
				keyStore.load(keystoreFileInputStream, credentials.getTruststorePassword().toCharArray());
			}
			...
			while (aliases.hasMoreElements()) {
				...
				message
						...
						.append("<BR>")
						.append("Serial: ").append(x509Certificate.getSerialNumber())
						.append("<BR>")
						.append("IssuerDN ").append(x509Certificate.getIssuerDN())
						.append("<BR>")
						.append("From: ").append(x509Certificate.getNotBefore())
						.append("<BR>")
						.append("To: ").append(x509Certificate.getNotAfter())
						.append("<BR><BR>");
			}
		...
		return new SelfTestResult(truststorePath, status, "Truststore", message.toString(), (status==Status.ERROR? message.toString() :null), stacktrace, stop-start + " ms", DESC_TRUSTSTORE);
	}
	...
	private SelfTestResult testFlameKeyStore() {
		...
		String endpoint = System.getProperty("no.nav.sdp-kanal.ebms.configdir")
				+ fileSeparator + "src/test/resources/flame" + fileSeparator
